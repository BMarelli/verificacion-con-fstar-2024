...
let raro (b: bool) : Type = if b then int else string
...
let f_raro (x: bool) : raro x = if x then 123 else "hola"
...
type yy (a: Type) (b: Type) = a & b

type oo (a: Type) (b: Type) = either a b
...
let comm_yy (#a #b: Type) :  yy a b -> yy b a = fun p -> (snd p, fst p)
...
let neutro_yy (#a: Type) :  yy a verd -> a = fun (x, _) -> x

let neutro_yy' (#a: Type) :  a -> yy a verd = fun (x: a) -> (x, vv)
...
let ex_falso (#a: Type) (f: falso) : a = match f with
...
let distr_yyoo_1 (#a #b #c: Type) :  yy a (oo b c) -> oo (yy a b) (yy a c) =
  function
  | x, Inl y -> Inl (x, y)
  | x, Inr z -> Inr (x, z)

let distr_yyoo_2 (#a #b #c: Type) :  oo (yy a b) (yy a c) -> yy a (oo b c) =
  function
  | Inl (x, y) -> (x, Inl y)
  | Inr (x, z) -> (x, Inr z)

let distr_ooyy_1 (#a #b #c: Type) :  oo a (yy b c) -> yy (oo a b) (oo a c) =
  function
  | Inl x -> (Inl x, Inl x)
  | Inr (y, z) -> (Inr y, Inr z)

let distr_ooyy_2 (#a #b #c: Type) :  yy (oo a b) (oo a c) -> oo a (yy b c) =
  function
  | Inl x, _ -> Inl x
  | _, Inl x -> Inl x
  | Inr y, Inr z -> Inr (y, z)

let modus_tollens (#a #b: Type) :  (a -> b) -> (no b -> no a) =
  fun f nob a -> nob (f a)



(* Vale la recíproca? *)
(* NO!. No se puede construir una funcion `a -> b` desde `b -> a` *)

let modus_tollendo_ponens (#a #b: Type) :  (oo a b) -> (no a -> b) =
  function
  | Inl a -> fun f -> (f a)
  | Inr b -> fun _ -> b
...
let modus_ponendo_tollens (#a #b: Type) :  no (yy a b) -> (a -> no b) =
  fun nab a b -> nab (a, b)
...
let modus_tollens_ponendo (#a #b: Type) :  (a -> no b) -> no (yy a b) =
  fun f (a, b) -> (f a) b
...
let demorgan1_vuelta (#a #b: Type) :  no (yy a b) -> oo (no a) (no b) = admit ()
...
let demorgan2_vuelta (#a #b: Type) :  no (oo a b) -> yy (no a) (no b) =
  fun naob -> ((fun a -> naob (Inl a)), (fun b -> naob (Inr b)))



(* P y no P no pueden valer a la vez. *)

let no_contradiccion (#a: Type) : no (yy a (no a)) = fun (x, f) -> f x
...
let elim_triple_neg (#a: Type) :  no (no (no a)) -> no a = fun f a -> f (fun g -> g a)



(* Ejercicio. ¿Se puede en lógica intuicionista? *)
(* NO!. No puedo generar p /\ q *)

let ley_impl1 (p q: Type) :  (p -> q) -> oo (no p) q = admit ()
...
let ley_impl3 (p q: Type) :  no (p -> q) -> yy p (no q) = admit ()



(* Ejercicio. ¿Se puede en lógica intuicionista? *)

let ley_impl4 (p q: Type) :  yy p (no q) -> no (p -> q) = fun (p, nq) f -> nq (f p)
...
let edn_implica_lte (edn: eliminacion_doble_neg) (#a: Type) : oo a (no a) = admit ()
